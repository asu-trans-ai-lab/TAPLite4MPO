...
# Golden-Path front door: three gates (simple first) + the dataset ladder
GATES = [
    ("1 · Can I run?", "blockers only", "#c0392b",
     "missing files · schema · topology · missing zones · undeclared conventions"),
    ("2 · Can I trust it?", "is the answer right?", "#d68910",
     "capacity convention · period & PLF · VDF · demand unit · allowed-use · toll · validation target"),
    ("3 · Can I improve it?", "go further", "#1e8449",
     "QVDF duration · path output · binary demand · super-zones · skims · dashboards"),
]
LADDER = [
    ("1", "Chicago Sketch", "minimum runnable — BPR, one period", "kernel/data_sets/03_chicago_sketch", "✅"),
    ("2", "Chicago Regional", "scale — binary demand, convergence, super-zones", "kernel/data_sets/04_chicago_regional", "✅"),
    ("3", "ARC Atlanta", "agency reproduction — mapping, PLF, classes, validation", "examples/arc_atlanta", "✅"),
    ("4", "Chicago Downtown OSM", "public quick start (OSM→GMNS)", "planned", "⏳"),
]
...
def render_html():
    ...
    gate_cards = "".join(
        f"<div class=gcard style='border-top:4px solid {col}'>"
        f"<div class=gq style='color:{col}'>{_esc(q)}</div>"
        f"<div class=gs>{_esc(sub)}</div><div class=gd>{_esc(cov)}</div></div>"
        for q, sub, col, cov in GATES)
    ladder_rows = "".join(
        f"<tr><td>{m} <b>{r}</b></td><td>{_esc(name)}</td><td>{_esc(teach)}</td>"
        f"<td><code>{_esc(loc)}</code></td></tr>"
        for r, name, teach, loc, m in LADDER)
    front = f"""
    <section class=front>
      <p class=lead>A shapefile + an OD matrix is <b>not yet an assignment model</b> — it can't
      state the capacity convention, period, PLF, VDF, user classes, tolls, or validation
      target. <b>TAPLite will not guess them.</b> Onboarding makes them explicit, converts to
      GMNS, runs, and validates.</p>
      <h3>The three questions</h3>
      <div class=gates>{gate_cards}</div>
      <h3>Start small — the dataset ladder</h3>
      <table class=ladder><tr><th>rung</th><th>example</th><th>teaches</th><th>location</th></tr>
      {ladder_rows}</table>
      <p class=note>Full write-ups: <code>docs/GOLDEN_PATH_CHECKLIST.md</code> ·
      <code>docs/DATASET_LADDER.md</code>. The detailed, gated journey follows ↓</p>
      <h3 class=detailh>The full path in detail — 7 stages</h3>
    </section>"""
 ...
 *{{box-sizing:border-box}} body{{font:15px/1.6 system-ui,Segoe UI,Arial;margin:0;color:#1d2733;background:#eef2f6}}
 ...
 .front{{margin-bottom:6px}} .lead{{font-size:16px;color:#243}}
 .front h3{{font-size:15px;margin:1.4em 0 .5em;color:#123a6b}} .detailh{{border-top:1px solid #dde3ea;padding-top:14px}}
 .gates{{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px}}
 .gcard{{background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:10px 14px;box-shadow:0 1px 3px #0001}}
 .gq{{font-weight:600;font-size:15px}} .gs{{color:#667;font-size:12px;margin:2px 0 6px}} .gd{{font-size:12.5px;color:#445}}
 .ladder code{{font-size:11.5px}} .note{{font-size:12.5px;color:#667}}
 @media(max-width:740px){{.cols,.gates{{grid-template-columns:1fr}}}}
...
{nav}</div>
<div class=wrap>{front}{cards}</div>
