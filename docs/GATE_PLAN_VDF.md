...
## Precise setup (as used in the previous the agency Cube run)
...
4 periods each (AM/MD/PM/NT). Each the agency link carries a per-period code
...
Source: Feng Liu 2024-03-21 conic VDF table (canonical the agency FT mapping).
...
PREPROCESSING (run once, before ANY assignment gate — `agency_run/prep_network.py`):
...
Two STANDING RULES for D–G (memory/time + validation discipline):
  ...
  • **Always validate on SF + CS multimodal first, then the agency** (small-network-first).

- **Gate D — Static path-based**: route_output on, SOV path columns only; reproduce
  link volume via per-OD SOV paths (column generation). SF/CS → the agency.
